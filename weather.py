...
st.set_page_config(page_title="zenibyte Weather Dashboard", page_icon="🌤️", layout="wide")

# Custom UI Styling (Nature Background, Cards, Sidebar aur Footer ke liye)
st.markdown("""
    <style>
    ...
    /* Sidebar styling for transparency and blur */
...
# --- SIDEBAR SEARCH ENGINE & NAVIGATION ---
st.sidebar.markdown("## 🔍 zenibyte Control Panel")

# 1. Page Navigation
menu_opt = st.sidebar.radio(
    "Go To Page:",
    ["🏠 Home Page", "ℹ️ About App", "🌍 Country Overview", "👨‍💻 Developer Corner"]
)
...
country_select = st.sidebar.selectbox(
    "Select Country:",
    ["India 🇮🇳", "United States 🇺🇸", "United Kingdom 🇬🇧", "Australia 🇦🇺", "Canada 🇨🇦", "United Arab Emirates 🇦🇪"]
)

st.sidebar.markdown("---")
st.sidebar.info("💡 Pro Tip: Select your preferred page and input details to get instant data points.")

# --- MAIN CONTENT AREA ---
if menu_opt == "🏠 Home Page":
    # Top Welcome Brand / Owner Name
    st.markdown("<div class='dev-badge'>🚀 zenibyte Official Product</div>", unsafe_allow_html=True)
    ...
    if st.button("Mausam Ka Haal Dekhein"):
        try:
            ...
            if "results" in geo_response:
                ...
                st.success(f"📍 Location Roadmap: {location_name}, {state}, {country} ({country_select})")
    ...
    st.title("ℹ️ About zenibyte Suite")
    st.markdown("""
    Welcome to the official **zenibyte Live Weather & Road Dashboard**. This application utilizes high-fidelity 
    open-source meteorology APIs to deliver live metrics across the globe.
    
    ### Key Highlights:
    * **Real-time Engine:** Instant telemetry for temperature, atmospheric pressure, and moisture parameters.
    * **Dynamic Air Quality Mapping:** Uses the US-AQI standards to dynamically color-code environment safety indexes.
    * **Granular Forecasts:** Hourly distribution windows tracking upcoming rainfall probability matrices.
    """)

elif menu_opt == "🌍 Country Overview":
    st.title("🌍 Global Country Specific Data Analysis")
    st.write(f"You have selected: **{country_select}**")
    st.info(f"Currently tracking specialized geo-indices for regions under {country_select}. Adjust the dropdown menu on the left sidebar to swap between configurations instantly.")

elif menu_opt == "👨‍💻 Developer Corner":
    st.title("👨‍💻 System Architecture & Developer Details")
    st.success("App Owner & Primary Architect: **Karan Roy**")
    st.markdown("""
    This complete ecosystem has been modeled, styled, and compiled by **Karan Roy** under the **zenibyte** brand umbrella. 
    Every style stylesheet element and functional routine runs autonomously via secure integration pipelines.
    """)

# --- PERSISTENT DEVELOPER FOOTER WITH ALL RIGHTS RESERVED ---
st.markdown("""
    <div class='footer'>
        © 2026 zenibyte. All Rights Reserved. | Designed & Developed by Karan Roy
    </div>
""", unsafe_allow_html=True)
